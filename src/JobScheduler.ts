...
export class JobScheduler {
   ...
   public scheduleJobs(config: Config) {
      // unschedule all scheduled job tasks in the background
      this.unscheduleAll();

      // schedule all configured jobs
      for (const jobEntry of config.jobs) this.scheduleJob(jobEntry, config.defaults);
   ...
   public scheduleJob(job: Job, defaults: Defaults = {}) {
      ...
      if (job.enabled === false) throw new Error(`Cannot schedule disabled job '${job.id}'`);
      if (this.runnerMap.has(job.id)) throw new Error(`Job '${job.id}' already scheduled!`);
    ...
    * @param cb callback
    */
   public onJobScheduled(cb: (job: Job) => void) {
    ...
    * - `COPIED`: a source file has been copied to `path` (emitted by job types `move` and `copy`)
    * - `DELETED`: a source file (`path`) has been deleted by the Job (emitted  by job types: `move` and `delete`)
    * - `ARCHIVED`: selected source files has been archived to `path` (emitted  by job type `archive`)
    * - `PRUNED` : a target file (`path`) has been pruned due to expired retention
    * Note: Events are ony emitted, if job config property `verbose` is set to `true`
    * @param cb callback
    */
